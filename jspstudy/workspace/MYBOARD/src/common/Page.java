...
public class Page {
	...
	private int recordPerPage = 5;    // 한페이지에 표시하는 게시글 수   
	...
	public String getPageEntity(String path) {
		...
		if(page <= 1) {
			...
			if( path.contains("?")) {
				sb.append("<a class=\"page_prev\"  href=\""+path+"&page="+(page-1)+"\">PREV</a>&nbsp;");
			} else {
				sb.append("<a  class=\"page_prev\" href=\""+path+"?page=" + (page - 1) + "\">PREV</a>&nbsp;");
			}
		}
		...
		if(page == totalPage) {
			...
			if(path.contains("?")){
				sb.append("<a class=\"page_next\"  href=\""+path +"&page="+(page+1)+"\">NEXT</a>");
			}
			 else {
					sb.append("<a  class=\"page_next\"  href=\""+path+"?page=" + (page+1) + "\">NEXT</a>&nbsp;");
				}
