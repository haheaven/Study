...
<%
	...
	if(check.equals("agree") ){                   // 회원가입 동의시 회원정보 파일로 저장하기 
		...
		//파일의 이름을 가지고 가야지 파일읽기가능하다. -->  filename 세션에 저장 
		session.setAttribute("filename", filename);
		response.sendRedirect("quiz08d.jsp");       //ct의 페이지에 대한 변화가 있어야하므로 redirect 사용 
		
	} else {
		response.sendRedirect("quiz08e.jsp");  // 가입 안했다는 페이지 
	}
